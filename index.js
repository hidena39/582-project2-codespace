...
app.post("/each-store/:storename/item", (req, res) => {
  ...
  async function run() {
    try {
      ...
      const result = await stores.updateOne({ _id: store._id }, { $set: { categories: store.categories } });
      console.log(result);
      res.send(result);
    } finally {
      // Ensures that the client will close when you finish/error
...
app.post("/deleteOneStore", (req, res) => {
  ...
  async function run() {
    try {
      ...
      const database = client.db("mongodemo");
      const stores = database.collection("stores");   
      const result = await stores.findOneAndDelete({ storename: storeToDelete });
      console.log(result);
      res.send(result);
    } finally {
      // Ensures that the client will close when you finish/error
  ...
  async function run() {
    try {
      ...
      const result = await stores.updateOne(
        { _id: store._id}, { $set: { categories:findCategories } }
        );
      console.log(result);
      res.send(result);
    } finally {
...
app.post("/each-store/:storename/deleteitem", (req, res) => {
  ...
  async function run() {
    try {
      ...
      const result = await stores.updateOne(
        { _id: store._id,"categories.category":categoryToDeleteItem }, { $set: { "categories.$.items":findItems } }
        );
      //$ is like [i]
      console.log(result);
      res.send(result);
...
app.post("/each-store/:storename/changestatus", (req, res) => {
  ...
  console.log(categoryToChangeStatus,  itemToChangeStatus, changedPurchaseStatus, markWhoBought);
  async function run() {
    try {
      const database = client.db("mongodemo");
      const stores = database.collection("stores");
      ...
      const result = await stores.updateMany(
        { _id: store._id}, { $set: { "categories.$[category].items.$[item].purchased":changedPurchaseStatus , "categories.$[category].items.$[item].time":purchasedDate, "categories.$[category].items.$[item].byWho":markWhoBought }}, {arrayFilters:[{"category.category":categoryToChangeStatus},{"item.item":itemToChangeStatus}]} 
        );
      console.log(result);
      res.send(result);
    } finally {
